...
public class UserDAO implements IUserDAO {
    ...
    private static final String FIND_USERS_SQL = "select * from users where lower(country) like ? ";
    ...
    @Override
    public boolean deleteUserPro(int id) throws SQLException {
        boolean rowDeleted;
        String sql = "{call remove_by_id(?)}";
    ...
    @Override
    public boolean updateUserPro(User user) throws SQLException {
        boolean rowUpdated;
    ...
    public List<User> selectUserByCountry(String cou) {
        // using try-with-resources to avoid closing resources (boiler plate code)
        String countri = '%' + cou +'%';
        List<User> users = new ArrayList<>();
        // Step 1: Establishing a Connection
        try (Connection connection = getConnection();
            ...
            preparedStatement.setString(1, countri);
            // Step 3: Execute the query or upadte query
            ResultSet rs = preparedStatement.executeQuery();
